...
int main()
{
    vector<string> sLine, arrMovePiece;
    string state;
    int N, iNumber;
    cin >> N;
    cin.ignore();
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        board = setNewMoves(board, arrMovePiece[i], FEN[1]);
        iNumber = stoi(FEN[2]);

        string sAns = BoardtoString(board, FEN[1], iNumber);
        string sCurrstate = getCurrentState(board);

        cout << sAns << endl;
        cout << sCurrstate << endl;
    }
