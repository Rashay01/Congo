...
int EvaluateThenewBoard(array<array<string, 7>, 7> board, string Player)
{
    vector<string> turns = getBestMove(board, "w");
    vector<string> bmoves = getBestMove(board, "b");
    int iRaw_score, iMaterial_score, iMobility_Score, iAttack_score;
    string state = getCurrentState(board);
    iMaterial_score = getBoardMaterialScore(board, Player);
    iMobility_Score = getBoardMobilityScore(board, Player, turns, bmoves);
    iAttack_score = getBoardAttackScore(board, Player, turns, bmoves);
    ...
    {
        iRaw_score = iMaterial_score + iMobility_Score + iAttack_score;
    }
...
vector<string> SortMoves(array<array<string, 7>, 7> board, string Player)
{
    vector<int> score;
    vector<string> moves, result;
    moves = getBestMove(board, Player);

    for (int i = 0; i < moves.size(); i++)
    {
        score.push_back(EvaluateThenewBoard(setNewMoves(board, moves[i], Player), Player));
    }
    vector<string> newListA, newListB = moves;
    int N = min(10, (int)moves.size());
    for (int i = 0; i < N; i++)
    {
        int max = -100000, maxLocation = 0;
        ...
        score[maxLocation] = -10000;
        newListA.push_back(moves[maxLocation]);
        newListB[maxLocation] = "";
    }

    result.insert(result.end(), newListA.begin(), newListA.end());
    result.insert(result.end(), newListB.begin(), newListB.end());
    return result;
}
...
int findMinMax(array<array<string, 7>, 7> Board, int depth, string Player)
{
    ...
    for (int m = 0; m < moves.size(); m++)
    {
        ...
        MInMaxValue = -findMinMax(newBoard, depth - 1, sNewPlayer);
        value = max(value, MInMaxValue);
    }
    return value;
}

int AlphaBeta(array<array<string, 7>, 7> board, int depth, string Player, int alpha, int beta)
{
    array<array<string, 7>, 7> newboard;
    vector<string> moves;
    string state = getCurrentState(board);
    ...
    if (state != "Continue" || depth == 0)
    {
        return EvaluateThenewBoard(board, Player);
    }

    moves = getBestMove(board, Player);
    moves = SortMoves(board, Player);
    for (int m = 0; m < moves.size(); m++)
    {
        if (moves[m] == "")
        {
        ...
        iAphaValue = -AlphaBeta(newboard, depth - 1, sNewPlayer, -beta, -alpha);
        if (iAphaValue >= beta)
        {
            return beta;
        }
...
int main()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        int AlphaboardEvaluation = AlphaBeta(board, 4, FEN[1], -1000000, 1000000);

        cout << AlphaboardEvaluation << endl;
    }
